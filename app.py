...
def seats(trip_id):
    ...
    for bus in buses:
        ...
        bus_data.append({"id": bus_id, "seats_count": seats_count, "booked": booked})
    conn.close()
...
def book():
    ...
    conn.commit()
    conn.close()
    return render_template("success.html", booking_id=booking_id, phone=SUPPORT_PHONE)

@app.route("/admin")
def admin():
    conn = sqlite3.connect("database.db")
    c = conn.cursor()
    c.execute("SELECT b.id, b.bus_id, b.seat_number, b.name, b.phone, bu.trip_id FROM bookings b JOIN buses bu ON b.bus_id=bu.id")
    data = c.fetchall()
    conn.close()
